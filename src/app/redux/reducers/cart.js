...
import { addMoney, subtractMoney, multiplyMoney} from '@/utils/moneyCalculations';
const initialState = {
    articles: [],
    numberOfArticles: 0,
    articlesSum: 0,
...
export const cartSlice = createSlice({
    ...
    reducers: {
       addArticleToCart: (state, action) => {
           ...
           state.articlesSum = addMoney(state.articlesSum, multiplyMoney(action.payload.quantity, action.payload.foodPrice))

        },
        incrementArticleQuantity: (state, action) => {
            const {index, increment } = action.payload
            state.articles[index].quantity =  state.articles[index].quantity + increment
            state.numberOfArticles = state.numberOfArticles + increment
            state.articlesSum = addMoney(state.articlesSum, multiplyMoney(state.articles[index].foodPrice, increment))
