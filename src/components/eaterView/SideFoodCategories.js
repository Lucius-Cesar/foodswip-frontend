...
export default function SideFoodCategories({foodCategories, activeFoodCategory}) {
    return (
        <div className ="flex flex-col justify-center h-screen w-screen sm:sticky sm:top-0 sm:w-80">
            ...
            <div className = "flex flex-col justify-center items-center overflow-y-auto  h-5/6 w-screen sm:w-full sm:border-r-2 border-light-grey">
                {foodCategories.map((foodCategory,i) => <h1 className ={`font-bold ${foodCategory === activeFoodCategory ? "text-primary":"text-medium-grey"} text-3xl mt-3 mb-3`} key = {i}>{foodCategory}</h1>)}
            </div>
        </div>
    )
  }