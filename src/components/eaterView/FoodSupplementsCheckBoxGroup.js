export default function FoodSupplementsCheckBoxGroup({label, items, chosenSupplements, setChosenSupplements}) {
  const onChangeCheckBox = (event, index) =>{
    ...
    if(event.target.checked){
      ...
      const concernedSupplementDeleteIndex = updatedChosenSupplements.findIndex(supplement => supplement.value === concernedSupplement.value && supplement.price === concernedSupplement.price)
          ...
          <div key={i} className="relative flex items-start">
            ...
            <div className="ml-3 text-sm leading-6">
              ...
              <span id={`${item.value}-description`} className="text-gray-500">
                <span className="sr-only">{item.value} </span>  {item.price && `(+ ${item.price} €)`}
              </span>
            </div>
          </div>
        ))}
