...
export default function FooterSettings({
  ...
  return (
    <div className="h-18 pt-6 sm:p-4 sm:pe-24 text-center sm:text-end">
      {restaurant.isLoading ? (
        <LoadingSpinner className="bg-success" />
      ) : (
        <button
          type="button"
          className="inline-flex items-center gap-x-2 rounded-md bg-success px-3.5 py-2.5 text-lg font-semibold text-white shadow-sm hover:hover:opacity-90 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:opacity-90"
          onClick={() => {
            if (
              ...
              dispatch(postRestaurantSettings(restaurantPayload));
            } else {
              setSaveError(true);
            }
          }}
